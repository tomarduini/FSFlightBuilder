        ...
        internal static FSPaths GetInstallPaths(FSPaths fsPaths)
        {
            ...
            try
            {
                ...
                if (key != null)
                {
                    ...
                    if (File.Exists(Path.Combine(fsPath, "steam_api.dll")))
                    {
                        //                    fsPaths.Add(FlightSimType.FSXSE, fsPath);
                        if (string.IsNullOrEmpty(fsPaths.DefaultFSPath))
                        {
                            MessageBox.Show("FSX Steam Edition found.", "New Flight Sim Found", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        ...
                        Common.logger.Info("FSXSE location found. {0}", fsPath);
                        foundSteam = true;
                    }
                }
                if (!foundSteam)
                {
                    ...
                    if (string.IsNullOrEmpty(fsPaths.DefaultFSPath) && key != null)
                    {
                        var fsPath = key.GetValue("AppPath").ToString().Trim('\0');
                        ...
                        else
                        {
                            ...
                            fsPaths.DefaultFSPath = fsPath;
                            var appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                            fsPaths.AppDataPath = Directory.Exists(appDataPath + "\\Microsoft\\FSX-SE") ?
                                appDataPath + "\\Microsoft\\FSX-SE" :
                                string.Empty;
                            fsPaths.CustomFSPath = Directory.Exists(fsPath + "\\Addon Scenery") ?
                                fsPath + "\\Addon Scenery" :
                                string.Empty;
            ...
            {
                Common.logger.Error("Error getting FSXSE registry. Error is: {0}", ex.Message);
            }
            return fsPaths;
        }
